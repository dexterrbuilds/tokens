...
`GET /v2/execution/evaluate` grades a hypothetical trade against current sampled on-chain
depth: per-variant impact at each sampled size rung, plus an interpolated estimate and
grade at your exact amount. Use it for pre-trade guardrails, sizing, and picking the right
variant mint at a given size.

Both require the `execution:read` scope, granted to all keys by default.
